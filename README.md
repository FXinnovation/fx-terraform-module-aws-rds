...
| backup\_retention\_period | The days to retain backups for. Default 1 | `number` | `1` | no |
...
| db\_instance\_promotion\_tiers | List of number for failover Priority setting on instance level | `list(number)` | `null` | no |
...
| preferred\_backup\_window | The daily time range during which automated backups are created if automated backups are enabled. | `string` | `null` | no |
| preferred\_maintenance\_window | The window to perform maintenance in. | `string` | `null` | no |
...
| rds\_cluster\_parameter\_group\_parameters | List of map of parameter to add. | <pre>list(object({<br>    name         = string<br>    value        = string<br>    apply_method = string<br>  }))</pre> | `[]` | no |
...
| secuirty\_group\_tags | Tags to be merged to security group | `map(string)` | `{}` | no |
